...
export default function WaitNode({ step, onUpdate, onDelete }: WaitNodeProps) {
  return (
    <div className="flex items-center gap-2 border border-gray-300 dark:border-[#45475a] bg-gray-50 dark:bg-[#181825] rounded-full px-4 py-2 shadow-sm">
      <Clock className="w-4 h-4 text-gray-400 dark:text-[#7f849c]" />
      <Button
        variant="ghost"
        size="sm"
      ...
      <span className="text-sm font-medium text-gray-600 dark:text-[#a6adc8] min-w-[50px] text-center">
        {step.waitDays || 0}d
      </span>
      <Button
        variant="ghost"
      ...
      {(step.waitHours !== undefined && step.waitHours > 0) || step.waitDays === 0 ? (
        <>
          ...
          <span className="text-sm font-medium text-gray-600 dark:text-[#a6adc8] min-w-[40px] text-center">
            {step.waitHours || 0}h
          </span>
          <Button
            variant="ghost"
      ...
      <Button
        variant="ghost"
        size="sm"
        className="h-6 w-6 p-0 text-gray-400 dark:text-[#7f849c] hover:text-red-500 ml-1"
